...
const FALLBACK_DESTINATIONS = [
  {
    id: 'paris1889', era: 'Belle Époque',
    title: 'Paris 1889 — Belle Époque', period: 'Année 1889',
    ...
    price: 8900, currency: 'chronos-coins', duration: { min: 3, max: 14 }, difficulty: 'Débutant',
    ...
    warning: "Évitez d'évoquer les guerres mondiales.", color: '#c9a84c', rating: 4.9, bookings: 1243,
  },
  {
    id: 'cretace', era: 'Préhistoire',
    title: "Crétacé — -65 millions d'années", period: '-65 000 000 av. J.-C.',
    ...
    price: 15500, currency: 'chronos-coins', duration: { min: 2, max: 7 }, difficulty: 'Débutant (capsule)',
    ...
    warning: "Sortie de la capsule formellement interdite.", color: '#4aaa3a', rating: 4.8, bookings: 892,
  },
  {
    id: 'florence1504', era: 'Renaissance',
    title: 'Florence 1504 — La Renaissance', period: 'Année 1504',
    ...
    price: 11200, currency: 'chronos-coins', duration: { min: 5, max: 21 }, difficulty: 'Intermédiaire',
    ...
    warning: "Vaccinations temporelles obligatoires contre la peste.", color: '#c44a1a', rating: 4.7, bookings: 654,
  },
];
      ...
      {loading ? (
        ...
        <div className={styles.grid}>
          {destinations.map(dest => (
            <DestinationCard key={dest.id} destination={dest} onDetails={setSelected} onReserve={onReserve} />
          ))}
        </div>
      )}
    ...
    <div className={styles.detail}>
      ...
      <div className={styles.detailCols}>
        <div>
          ...
          {dest.highlights.map(h => <p key={h} className={styles.highlight} style={{ borderColor: dest.color }}>· {h}</p>)}
        </div>
        <div>
          <p className={styles.colLabel}>Dans le forfait</p>
          {dest.included.map(i => <p key={i} className={styles.included}>✓ {i}</p>)}
        </div>
      </div>
      <div className={styles.detailMeta}>
        <div className={styles.metaItem}><span className={styles.metaNum}>{dest.price.toLocaleString('fr-FR')} ⊕</span><span className={styles.metaLabel}>par personne</span></div>
        <div className={styles.metaItem}><span className={styles.metaNum}>{dest.duration.min}–{dest.duration.max} j</span><span className={styles.metaLabel}>durée</span></div>
        <div className={styles.metaItem}><span className={styles.metaNum}>{dest.difficulty}</span><span className={styles.metaLabel}>niveau</span></div>
        <div className={styles.metaItem}><span className={styles.metaNum}>★ {dest.rating}</span><span className={styles.metaLabel}>note</span></div>
      </div>
      <div className={styles.warning}><span>⚠ Avertissement temporel</span><p>{dest.warning}</p></div>
      <button className={styles.detailReserve} onClick={() => onReserve(dest.id)}>⟶ Réserver cette destination</button>
    </div>
  );
}
